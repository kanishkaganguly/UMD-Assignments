function [ out_img ] = stitch2(stitched_img, imgL, imgR)
% RIGHT SIFT %
imgR = single(imgR);
[fR,dR] = vl_sift(imgR);
%display_sift_features(fR);
% LEFT SIFT %
imgL = single(imgL);
[fL,dL] = vl_sift(imgL);
%display_sift_features(fL);
...
stitched_img = stitched_img + stitch(imgL, imgR, T);
out_img = stitched_img;
end